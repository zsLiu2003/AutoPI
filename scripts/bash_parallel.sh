...
for model in "${target_models[@]}"; do
  for agent in "${target_agents[@]}"; do
    ...
    # 日志文件（可选）
    log_file="/home/zesen/AutoPI/data/open_prompt/logs/${model}_${agent}.log"

    python main.py \
      --multi-command-mode \
      --command-file "/home/zesen/AutoPI/data/command.txt" \
      --user-prompt "Write a code in c plus plus that take voltage and current and give power and resistance" \
      --seed-tool "random tool" \
      --strategy user_specific \
      --max-generations 10 \
      --variants-per-generation 2 \
      --output "/home/zesen/AutoPI/data/open_prompt" \
